...
public class fastrpcgen {
    ...
    private Project parseIDLtoDDS(String idlFilename) {
        ...
        if (idlParseFileName != null) {
            ...
            // Load template to generate proxy user implementations.
            tmanager.addGroup("ProxyImplHeader");
            tmanager.addGroup("ProxyImplHeaderExample");
            tmanager.addGroup("ProxyImplSourceExample");
            ...
            if (returnedValue) {
                ...
                if (returnedValue && numberOfInterfaces > 0 && m_clientcode) {
                    ...
                    if (returnedValue = Utils.writeFile(m_outputDir + onlyFileName + "Proxy.h", maintemplates.getTemplate("ProxyHeader"), m_replace)) {
                        if (returnedValue = Utils.writeFile(m_outputDir + onlyFileName + "Proxy.cxx", maintemplates.getTemplate("ProxySource"), m_replace)) {
                            if (returnedValue = Utils.writeFile(m_outputDir + onlyFileName + "ProxyImpl.h", maintemplates.getTemplate("ProxyImplHeader"), m_replace)) {
                                project.addClientIncludeFile(onlyFileName + "Proxy.h");
                                project.addClientSrcFile(onlyFileName + "Proxy.cxx");
                                project.addClientIncludeFile(onlyFileName + "ProxyImpl.h");

                                if (m_exampleOption != null) {
                                    if (returnedValue = Utils.writeFile(m_outputDir + onlyFileName + "ProxyImplExample.h", maintemplates.getTemplate("ProxyImplHeaderExample"), m_replace)) {
                                        if (returnedValue = Utils.writeFile(m_outputDir + onlyFileName + "ProxyImplExample.cxx", maintemplates.getTemplate("ProxyImplSourceExample"), m_replace)) {
