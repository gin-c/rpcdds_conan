...
@init {
        ...
        String name = "";
        Token tk = null, tkoneway = null, tknotify = null;
        TypeCode retType = null;
        Vector<Pair<String, Token>> exceptions = null;
}
    :   ( op_attribute { tkoneway=$op_attribute.token; } )? ( op_attribute_notify { tknotify=$op_attribute_notify.token; } )?
        ...
        ID
        {
           ...
           // Set notify
             if(tknotify != null) {
                 operationObject.setNotify(true);

                 if(retType != null)
                 {
                     throw new ParseException(tknotify, ". Notify function cannot have a return type.");
                 }
             }
        }
...
op_attribute_notify returns [Token token = null]
@init {
    Token tk = _input.LT(1);
}
    :  KW_NOTIFY { $token = tk;}
