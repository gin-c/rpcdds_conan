...
RTPSProxyProcedureEndpoint::RTPSProxyProcedureEndpoint(RTPSProxyTransport &transport)
    : m_mutex(NULL),
      recv_mutex_(NULL),
      matched_mutex_(NULL),
...
int RTPSProxyProcedureEndpoint::initialize(const char *name, const char *writertypename, const char *writertopicname,
    ...
    if (m_mutex != NULL && recv_mutex_ != NULL && matched_mutex_ != NULL && matched_sub_cond_ != NULL &&
        matched_pub_cond_ != NULL) {
        if (createEntities(name, writertypename, writertopicname, readertypename, readertopicname) == 0) {
            // Not oneway
            if (m_reader != NULL) {
...
int RTPSProxyProcedureEndpoint::createEntities(const char *name, const char *writertypename,
            ...
            if (m_writer != nullptr) {
                ...
                if (readertypename != NULL) {
                    ...
                    if (m_reader != nullptr) {
                        ...
                        printf("ERROR<%s::%s>: Cannot create the data reader\n", CLASS_NAME, METHOD_NAME);
                    }
                } else {
                    return 0;
                }
            ...
            if (recvpoint != nullptr) {
                if (retWrite) {
                    ...
                    if (!recvpoint->received_) {
                        if (recvpoint->cond_.wait_for(lock, boost::chrono::milliseconds(m_transport.getTimeout())) !=
...
void RTPSProxyProcedureEndpoint::onNewDataMessage(eprosima::fastrtps::Subscriber *sub) {
    ...
    if (sub->takeNextData(data_, &info)) {
        if (info.sampleKind == eprosima::fastrtps::rtps::ALIVE) {
            ...
            // 接收 通知
            if (replyHeader->relatedRequestId().writer_guid().entityId().entityKind() ==
                EntityKind_t::ENTITY_KIND_BROADCAST) {
                void *data = m_create_data(m_dataSize);
                m_copy_data(data, data_);
                m_transport.getStrategy().getImpl()->schedule([ObjectPtr = &m_transport, this, data] {
                    ObjectPtr->process(this, data, m_destroy_data);
                });
            } else if (replyHeader->relatedRequestId().writer_guid().guidPrefix()[0] == m_proxyId.guidPrefix()[0] &&
                ...
                if (it != recv_threads.end()) {
                    ...
                    // Async task
                        ...
                        delete rp;
                        lock.unlock();
                        m_transport.getStrategy().getImpl()->schedule(boost::bind(&RTPSAsyncTask::execute, task));
                    }
                }
            }
        }
...
void RTPSProxyProcedureEndpoint::onSubscriptionMatched(eprosima::fastrtps::Subscriber * /*sub*/,
...
void RTPSProxyProcedureEndpoint::onPublicationMatched(eprosima::fastrtps::Publisher * /*pub*/,
                                                      eprosima::fastrtps::rtps::MatchingInfo &info) {
    boost::upgrade_lock<boost::shared_mutex> lock(*matched_mutex_);
    boost::upgrade_to_unique_lock<boost::shared_mutex> uniqueLock(lock);
